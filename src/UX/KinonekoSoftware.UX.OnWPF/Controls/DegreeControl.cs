...
namespace KinonekoSoftware.UX
{
    public class DegreeControl : Control
    {
        ...
        private Point[] _Position = new Point[32];
        ...
        protected override Size MeasureOverride(Size constraint)
        {
            ...
            var column = (int)(constraint.Width / (gap + size));
            var row    = (max + column - 1) / column;

            var w = double.IsInfinity(constraint.Width) ? column * size + (column - 1) * gap : constraint.Width;
            return new Size(w, row * 12 + (row - 1) * gap);
        }
        ...
        protected override void OnRender(DrawingContext dc)
        {
            ...
            for (int i = 0; i < max;)
            {
                for (var c = 0; c < column && i < max; c++)
                {
                    ...
                    var b = v > i && v > 0 ? fg : bg;
                    dc.DrawRoundedRectangle(b, null, new Rect(x, y, size, 9.01), r, r);
