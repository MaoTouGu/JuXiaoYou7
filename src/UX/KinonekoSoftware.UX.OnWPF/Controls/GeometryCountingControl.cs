...
{
    public class GeometryCountingControl : Panel
    {
        public static readonly DependencyProperty StrokeThicknessProperty = DependencyProperty.Register(
            nameof(StrokeThickness),
            typeof(int),
            typeof(GeometryCountingControl),
        ...
        public static readonly DependencyProperty MaximumProperty = DependencyProperty.Register(
            ...
            typeof(int),
            typeof(GeometryCountingControl),
            new FrameworkPropertyMetadata(0, FrameworkPropertyMetadataOptions.AffectsRender, OnInvalidateRenderState));

        private static void OnInvalidateRenderState(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            var c = (GeometryCountingControl)d;
            c.GenerateElement();
        }
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            var size = IconSize;
            var gap  = IconGap;
            var maxW = double.IsInfinity(availableSize.Width) ? 100 : availableSize.Width;
            ...
            //
            // 273.2 / 19 = 14;
            var per = (int)(maxW / (size + 4));           
            var line = (InternalChildren.Count + (per - 1)) / per;

            return new Size(
                            width: per * size,
                            height: line * size + (line - 1) * 8);
        }
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            var size = IconSize;
            var size2 = size + 2;
            var per = (int)(finalSize.Width / (size2));
            var c = 0;
            var r = 0;
            ...
            for (var i = 0 ;i < InternalChildren.Count; i++)
            {
                ...
                element.Arrange(new Rect(c * (size2), r * (size2), size2, size2));
