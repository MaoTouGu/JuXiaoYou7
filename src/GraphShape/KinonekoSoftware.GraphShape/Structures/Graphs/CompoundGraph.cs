...
namespace GraphShape
{
        ...
        where TEdge : IEdge<TVertex>
    {
            ...
            : base(graph?.AllowParallelEdges ?? throw new ArgumentNullException(nameof(graph)), graph.VertexCount)
        {
            ...
            foreach (var vertex in graph.CompoundVertices)
            {
        ...
        private bool AddChildVertexInternal( TVertex parent,  TVertex child, [NotNull, ItemNotNull] ICollection<TVertex> children)
        {
            if (_parentRegistry.TryGetValue(child, out var currentParent))
            {
        ...
        public int AddChildVertexRange(TVertex parent, IEnumerable<TVertex> children)
        {
            ...
            var count = AddVertexRange(childrenArray);
        ...
        public override bool RemoveVertex(TVertex vertex)
        {
            ...
            if (removed)
            {
                ...
                foreach (var pair in _childrenRegistry)
                {
