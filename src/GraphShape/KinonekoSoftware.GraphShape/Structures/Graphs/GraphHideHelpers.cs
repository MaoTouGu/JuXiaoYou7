...
namespace GraphShape
{
        ...
        where TEdge : IEdge<TVertex>
    {
        ...
        public GraphHideHelpers( IMutableBidirectionalGraph<TVertex, TEdge> managedGraph)
        {
        ...
        [Pure]
        [NotNull, ItemNotNull]
        private IEnumerable<TEdge> EdgesFor( TVertex vertex)
        {
        ...
        private HiddenCollection GetHiddenCollection( string tag)
        {
            if (!_hiddenCollections.TryGetValue(tag, out var collection))
            {
        ...
        private void OnVertexHidden( TVertex vertex)
        {
            Debug.Assert(vertex != null);
        ...
        private void OnVertexUnhidden( TVertex vertex)
        {
            Debug.Assert(vertex != null);
        ...
        private void OnEdgeHidden( TEdge edge)
        {
        ...
        private void OnEdgeUnhidden( TEdge edge)
        {
            Debug.Assert(edge != null);
        ...
        {
            var collection = GetHiddenCollection(tag);
        ...
        {
            UnhideVertex(vertex);
            _hiddenEdgesOf.TryGetValue(vertex, out var hiddenEdges);
            if (hiddenEdges != null)
            {
        ...
        private List<TEdge> GetHiddenEdgeListOf( TVertex vertex, bool createIfNotExists)
        {
            Debug.Assert(vertex != null);

            if (_hiddenEdgesOf.TryGetValue(vertex, out var hiddenEdges) || !createIfNotExists)
                return hiddenEdges;
        ...
        {
            var collection = GetHiddenCollection(tag);
            foreach (var vertex in collection.HiddenVertices)
            {
                UnhideVertex(vertex);
            }

            foreach (var edge in collection.HiddenEdges)
            {
                UnhideEdge(edge);
            }
