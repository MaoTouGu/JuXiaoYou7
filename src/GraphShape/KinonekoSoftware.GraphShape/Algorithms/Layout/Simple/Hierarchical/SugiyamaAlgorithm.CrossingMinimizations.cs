        ...
        where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
    {
        ...
        private void DoCrossingMinimizations()
        {
            ...
            do
            {
                ...
                crossings = Sweeping(0, _layers.Count - 1, 1, enableSameMeasureOptimization, out var c, ref phase);
                changed = c;
                if (crossings == 0)
                    break;
            ...
            ref int phase)
        {
            ...
            var layer = _alternatingLayers[startLayerIndex];
            if (layer is null)
            {
            ...
            for (var i = startLayerIndex; i != endLayerIndex; i += step)
            {
                ...
                var nextAlternatingLayer = _alternatingLayers[i + step];
                ...
                var crossCount = CrossingMinimizationBetweenLayers(
            ...
            int phase)
        {
            ...
            // 5
            var crossCount = DoCrossCountingAndOptimization(
                                                            alternatingLayer,
                                                            nextAlternatingLayer,
                                                            straightSweep,
                                                            enableSameMeasureOptimization,
            ...
            bool straightSweep)
        {
            ...
            while (vertexStack.Count > 0 && segmentContainerStack.Count > 0)
            {
                ...
                else
                {
                    ...
                    segmentContainer.Split(k, out var container1, out var container2);
            ...
            bool straightSweep)
        {
            ...
            for (var i = 0; i < alternatingLayer.Count; ++i)
            {
                ...
                for (var j = 0; j < segmentContainer.Count; ++j)
                {
                    ...
                    segmentContainer.Split(segment, out var container1, out var container2);
            ...
            int layerIndex)
        {
            ...
            {
                foreach (var edge in sparseCompactionGraphEdgesOfLayer)
                {
            ...
            int prevCrossCount)
        {
            ...
            var firstLayerSize = lastOnTopLayer.Position + (lastOnTopLayer is ISegmentContainer topContainer ? topContainer.Count : 1);
            var secondLayerSize = lastOnBottomLayer.Position + (lastOnBottomLayer is ISegmentContainer bottomContainer ? bottomContainer.Count : 1);

            var virtualEdgePairs = FindVirtualEdgePairs(topLayer, bottomLayer);
            var realEdges = FindRealEdges(topLayer);
            ...
            if (enableSameMeasureOptimization || reverseVerticesWithSameMeasure)
            {
                ...
                var sortedVertexList = ComputeSortedVertexList(
                                                               verticesWithSameMeasure,
                                                               reverseVerticesWithSameMeasure);

                var maxPermutations = SugiyamaLayoutParameters.MaxPermutations;
                do
                {
                    --maxPermutations;
                    ...
                    var crossCount = BiLayerCrossCount(edgePairs, firstLayerSize, secondLayerSize);
        ...
        [Pure]
        private bool Swap([NotNull, ItemNotNull] IList<SugiVertex> vertices,  IList<int> ranges)
        {
            var swapped = false;
