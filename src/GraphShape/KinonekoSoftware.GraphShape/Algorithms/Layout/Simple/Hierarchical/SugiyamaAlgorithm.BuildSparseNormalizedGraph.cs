...
namespace GraphShape.Algorithms.Layout
{
        ...
        where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
    {
        ...
        private void CreateInitialLayering()
        {
            ...
            for (var i = 0; i < layerTopologicalSort.LayerCount; ++i)
            {
                // Set the layer
                _layers.Add(layerTopologicalSort.Layers[i].ToList());

                // Assign the layer indexes
                foreach (var vertex in _layers[i])
                {
        ...
        private void MinimizeInitialLayersEdgeLength()
        {
            ...
            {
                var layer = _layers[i];
                foreach (var vertex in layer.ToArray())
                {
                    ThrowIfCancellationRequested();
                    ...
                    var newLayerIndex = _graph.OutEdges(vertex).Min(edge => edge.Target.LayerIndex - 1);
        ...
        {
            foreach (var edge in _graph.Edges.ToArray())
            {
        ...
        private static void AddSegment( SugiVertex pVertex,  SugiVertex qVertex)
        {
        ...
        private bool DoWHOptimizationStep()
        {
            ...
            // Insert a new layer
            var insertedLayerIndex = maxWidthLayerIndex + 1;
            double width = 0;
            double c = 0;
        ...
        private void CreateLayerWHOptimizationInfos()
        {
            ...
            foreach (var layer in _layers)
            {
                ...
                foreach (var vertex in layer)
                {
                    ...
                    if (_whOptVertexInfos.TryGetValue(vertex, out var vertexInfo) && vertexInfo.ValuePerCost >= 0)
                    {
                ...
                foreach (var vertexInfo in layerInfo.Vertices)
                {
                    ThrowIfCancellationRequested();
                ...
                foreach (var vertexInfo in verticesList)
                {
                    layerInfo.Vertices.Enqueue(vertexInfo);
                }
