...
namespace GraphShape.Algorithms.Layout
{
        ...
        where TGraph : IVertexAndEdgeListGraph<TVertex, TEdge>
    {
        ...
        private void CalculateOnlyIsolatedVerticesPositions()
        {
            ...
            var nbVerticesPerSide = (int) Math.Ceiling(Math.Sqrt(VisitedGraph.VertexCount));

            var maxWidth = _verticesSizes?.Max(v => v.Value.Width) ?? 0;
            var maxHeight = _verticesSizes?.Max(v => v.Value.Height) ?? 0;
            ...
            if (IsVerticalLayout())
            {
                foreach (var vertex in _isolatedVertices)
                {
        ...
        {
            foreach (var pair in _dummyVerticesOfEdges)
            {
                var routePoints = new Point[pair.Value.Count];
                for (var i = 0; i < pair.Value.Count; ++i)
                {
                    var vertex = pair.Value[i];
        ...
        {
            var isVerticalLayout = IsVerticalLayout();
        ...
        private void AssignEdgesRoutes(bool isVerticalLayout)
        {
            foreach (var edge in VisitedGraph.Edges)
            {
                ...
                var sourceVertex = _verticesMap[edge.Source];
                var targetVertex = _verticesMap[edge.Target];
                var notSwitched = sourceVertex.LayerIndex < targetVertex.LayerIndex;
                var sourceIndex = notSwitched ? 0 : 1;
                var targetIndex = notSwitched ? 1 : 0;
        ...
        private void AssignDummyVerticesEdgesRoutes(bool isVerticalLayout)
        {
            foreach (var pair in _dummyVerticesOfEdges)
            {
                var orthogonalRoutePoints = EdgeRoutes[pair.Key];
        ...
        private void CalculateLayerSizesAndPositions()
        {
            ...
            var layerDistance = Parameters.LayerGap;
                ...
                for (var j = vertexStart; j != vertexEnd; j += vertexStep)
                {
                    ...
                    switch (vertex.Type)
                    {
                        ...
                        case VertexTypes.QVertex when upperLowerEdges == UpperLowerEdges.Lower:
                            {
                                var neighborEdges = upperLowerEdges == UpperLowerEdges.Upper
                                ...
                                var c1 = (int)Math.Floor((neighborEdges.Length + 1) / 2.0) - 1;
                                var c2 = (int)Math.Ceiling((neighborEdges.Length + 1) / 2.0) - 1;
                                int[] medians;
                                if (c1 == c2)
                                {
                                    medians = new[] { c1 };
             ...
             SugiVertex v)
        {
            ...
            do
            {
                     ...
                     || leftRightMode == LeftRightMode.Right && _sparseCompactionGraph.OutDegree(w) > 0))
                {
                    ...
                    {
                        SugiVertex u;
                        var predecessor = leftRightMode == LeftRightMode.Left ? edge.Source : edge.Target;
