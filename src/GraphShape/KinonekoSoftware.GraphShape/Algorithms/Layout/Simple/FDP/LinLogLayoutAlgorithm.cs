...
namespace GraphShape.Algorithms.Layout
{
        ...
        where TGraph : IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        private sealed class LinLogEdge
        {
            ...
            public LinLogEdge( LinLogVertex target, double weight)
            {
                Target = target;
        ...
        protected override void InternalCompute()
        {
            ...
            var finalRepulsiveExponent = Parameters.RepulsiveExponent;
            var finalAttractionExponent = Parameters.AttractionExponent;

            for (var step = 1; step <= Parameters.MaxIterations; ++step)
            {
        ...
        private void MoveNodes( QuadTree quadTree)
        {
        ...
        private void MoveNode( QuadTree quadTree, int index)
        {
            var vertex = _vertices[index];
            var oldEnergy = GetEnergy(index, quadTree);

            // Compute direction of the move of the node
            GetDirection(index, quadTree, out var bestDirection);
                ...
                multiple /= 2)
            {
                ...
                var curEnergy = GetEnergy(index, quadTree);
        ...
        private void InitAlgorithm()
        {
            ...
            foreach (var vertex in _vertices)
            {
                ...
                foreach (var edge in VisitedGraph.InEdges(vertex.OriginalVertex))
                {
                    var weightedEdge = edge as WeightedEdge<TVertex>;
                    var weight = weightedEdge?.Weight ?? 1;
                ...
                foreach (var edge in VisitedGraph.OutEdges(vertex.OriginalVertex))
                {
                    var weightedEdge = edge as WeightedEdge<TVertex>;
                    var weight = weightedEdge?.Weight ?? 1;
        ...
        private void GetDirection(int index, QuadTree quadTree, out Vector direction)
        {
            ...
            var direction2 = AddRepulsionDirection(index, quadTree, ref direction);
        ...
        private double AddRepulsionDirection(int index, [CanBeNull] QuadTree quadTree, ref Vector direction)
        {
            ...
            var repulsionVector = quadTree.Position - vertex.Position;
            var distance = repulsionVector.Length;
            if (quadTree.Index < 0 && distance < 2.0 * quadTree.Width)
            {
                var direction2 = 0.0;
                foreach (var childTree in quadTree.Children)
                {
        ...
        {
            var vertex = _vertices[index];
            var gravitationVector = _barycenter - vertex.Position;
            var distance = gravitationVector.Length;
            var tmp = Parameters.GravitationMultiplier
        ...
        private double AddAttractionDirection(int index, ref Vector direction)
        {
            var direction2 = 0.0;
            var vertex = _vertices[index];
            foreach (var edge in vertex.Attractions)
            {
                // Avoid loop
                if (edge.Target == vertex)
                    continue;

                var attractionVector = edge.Target.Position - vertex.Position;
                var distance = attractionVector.Length;
                if (distance <= 0)
                    continue;

                var tmp = edge.AttractionWeight * Math.Pow(distance, Parameters.AttractionExponent - 2);
        ...
        private double GetRepulsionEnergy(int index, [CanBeNull] QuadTree quadTree)
        {
            ...
            var dist = (vertex.Position - quadTree.Position).Length;
        ...
        private double GetAttractionEnergy(int index)
        {
            ...
            foreach (var edge in vertex.Attractions)
            {
                ...
                var distance = (edge.Target.Position - vertex.Position).Length;
        ...
        private double GetGravitationEnergy(int index)
        {
            ...
            var distance = (vertex.Position - _barycenter).Length;
