...
namespace GraphShape.Algorithms.Layout
{
        ...
        where TGraph : IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        public ISOMLayoutAlgorithm( TGraph visitedGraph, [CanBeNull] ISOMLayoutParameters parameters = null)
        ...
        protected override void InternalCompute()
        {
            ...
            for (var epoch = 0; epoch < Parameters.MaxEpochs; ++epoch)
            {
                ...
                // Update Parameters
                var factor = Math.Exp(-1 * Parameters.CoolingFactor * (1.0 * epoch / Parameters.MaxEpochs));
        ...
        private void AdjustVertex( TVertex closest)
        {
            ...
            while (_queue.Count > 0)
            {
                ...
                var currentData = _isomDataDict[current];
                var position = VerticesPositions[current];

                var force = _tempPos - position;
                var factor = _adaptation / Math.Pow(2, currentData.Distance);
                ...
                if (currentData.Distance < _radius)
                {
                    ...
                    foreach (var neighbor in VisitedGraph.GetNeighbors(current))
                    {
                        ThrowIfCancellationRequested();

                        var neighborData = _isomDataDict[neighbor];
        ...
        private TVertex GetClosest(Point tempPos)
        {
            ...
            foreach (var vertex in VisitedGraph.Vertices)
            {
                var d = (tempPos - VerticesPositions[vertex]).Length;
                if (d < distance)
                {
                    closest = vertex;
                    distance = d;
