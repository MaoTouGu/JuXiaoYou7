        ...
        where TGraph : IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        private void RemoveTreeNodesFromRootGraph()
        {
            ...
            for (var i = 0; removed; ++i)
            {
                ...
                foreach (var vertex in Levels[0])
                {
                    ...
                    var edge = _compoundGraph.InDegree(vertex) > 0
                ...
                if (removed && _removedRootTreeNodeLevels.Count > 0)
                {
                    ...
                    foreach (var data in _removedRootTreeNodeLevels.Peek())
                    {
        ...
        private void InitMovableParentOfFixedVertices([NotNull, ItemNotNull] Queue<TVertex> movableParentUpdateQueue)
        {
            while (movableParentUpdateQueue.Count > 0)
            {
                ...
                var fixedLayoutCompoundVertex = movableParentUpdateQueue.Dequeue();

                // Find the not fixed predecessor
                var movableVertex = fixedLayoutCompoundVertex;
                ...
                foreach (var childVertex in _compoundGraph.GetChildrenVertices(fixedLayoutCompoundVertex))
                {
                    var data = _verticesData[childVertex];
        ...
        private void InitSimpleVertices( IDictionary<TVertex, Size> verticesSizes)
        {
            foreach (var vertex in _compoundGraph.SimpleVertices)
            {
                verticesSizes.TryGetValue(vertex, out var vertexSize);

                VerticesPositions.TryGetValue(vertex, out var position);
            ...
            [NotNull, ItemNotNull] Queue<TVertex> movableParentUpdateQueue)
        {
            for (var i = Levels.Count - 1; i >= 0; --i)
            {
                foreach (var vertex in Levels[i])
                {
                    ...
                    verticesBorders.TryGetValue(vertex, out var border);

                    verticesSizes.TryGetValue(vertex, out var vertexSize);
                    layoutTypes.TryGetValue(vertex, out var layoutType);
                    ...
                    var childrenData = children.Select(v => _verticesData[v]).ToList();
