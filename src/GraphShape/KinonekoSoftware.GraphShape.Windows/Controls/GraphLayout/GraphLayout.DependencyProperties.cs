...
namespace GraphShape.Controls
{
        ...
        where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        private static void OnEdgeRoutingAlgorithmTypePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs args)
        {
            ...
            var newAlgorithmType = args.NewValue is null ? string.Empty : args.NewValue.ToString();
        ...
        private static void OnHighlightAlgorithmFactoryPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs args)
        {
            ...
            var highlightMethod = graphLayout.HighlightAlgorithmType;
        ...
        private static void OnHighlightAlgorithmTypePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs args)
        {
            ...
            var parameters = graphLayout.HighlightAlgorithmFactory.CreateParameters(
                                                                                    newAlgorithmType,
        ...
        private static void OnLayoutModePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs args)
        {
            ...
            if (graphLayout.Graph is IMutableBidirectionalGraph<TVertex, TEdge> mutableGraph)
            {
                ...
                var wasCompoundMode = IsCompoundModeInternal(oldMode, graphLayout.Graph);
                var isCompoundMode = IsCompoundModeInternal(newMode, graphLayout.Graph);
        ...
        protected static void OnGraphPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs args)
        {
            ...
            if (args.NewValue is not TGraph newGraph)
            {
                graphLayout.LayoutMode = LayoutMode.Automatic;
                return;
            }

            graphLayout.OnGraphChanged(args.OldValue as TGraph, args.NewValue as TGraph);
        ...
        {
            if (!IsCompoundModeInternal(LayoutMode, oldGraph)
                && oldGraph is IMutableBidirectionalGraph<TVertex, TEdge> oldMutableGraph)
            {
                // Unsubscribe to events of the old graph mutations
                UnregisterMutableGraphHandlers(oldMutableGraph);
            }
            
            if (!IsCompoundMode && newGraph is IMutableBidirectionalGraph<TVertex, TEdge> newMutableGraph)
            {
                // Subscribe to events of the new graph mutations
                RegisterMutableGraphHandlers(newMutableGraph);
            }

            OnRelayoutInduction(true);
        }

        protected void RegisterMutableGraphHandlers( IMutableBidirectionalGraph<TVertex, TEdge> graph)
        {
        ...
        protected void UnregisterMutableGraphHandlers( IMutableBidirectionalGraph<TVertex, TEdge> graph)
        {
            graph.VertexAdded -= OnMutableGraphVertexAdded;
            graph.VertexRemoved -= OnMutableGraphVertexRemoved;
            graph.EdgeAdded -= OnMutableGraphEdgeAdded;
        ...
        private void OnHighlightTriggered( object sender,  HighlightTriggeredEventArgs args)
        {
        ...
        private void OnEdgeHighlightTriggered( EdgeControl edgeControl,  HighlightTriggeredEventArgs args)
        {
        ...
        protected void OnRelayoutInduction(bool tryKeepControls)
        {
