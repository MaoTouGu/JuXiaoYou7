...
namespace GraphShape.Controls
{
        ...
        where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        [Pure]
        protected static bool IsCompoundModeInternal(Algorithms.Layout.LayoutMode mode)
        {
            return mode == Algorithms.Layout.LayoutMode.Compound;
        }

        [Pure]
        protected static bool IsCompoundModeInternal(LayoutMode mode, [CanBeNull] TGraph graph)
        {
             ...
             IDictionary<TVertex, Size> sizes)
        {
            ...
            {
                if (!positions.TryGetValue(vertex, out var position) || !sizes.TryGetValue(vertex, out var size))
                    continue;
        ...
        protected void RunAsynchronousLayout()
        {
            CancelLayout();
        ...
        protected void RunSynchronousLayout()
        {
            ...
            IDisposable AlgorithmScope(ILayoutAlgorithm<TVertex, TEdge, TGraph> algorithm)
            {
                ...
                var showAllStates = ShowAllStates;
                if (showAllStates)
                {
        ...
        [Pure]
        [CanBeNull]
        protected IDictionary<TVertex, Point> GetOldVerticesPositions(bool continueLayout)
        {
            if (ActualLayoutMode == Algorithms.Layout.LayoutMode.Simple)
            {
        ...
        [Pure]
        protected IDictionary<TVertex, Point> GetLatestVerticesPositions()
        {
            ...
            if (ActualLayoutMode == Algorithms.Layout.LayoutMode.Simple)
            {
                foreach (KeyValuePair<TVertex, VertexControl> pair in VerticesControls)
                {
                    var x = GetX(pair.Value);
                    var y = GetY(pair.Value);
            ...
            else
            {
                ...
                foreach (KeyValuePair<TVertex, VertexControl> pair in VerticesControls)
                {
                    var position = pair.Value.TranslatePoint(topLeft, this);
        ...
        [Pure]
        private static System.Windows.Point GetRelativePosition( VertexControl vertexControl, [CanBeNull] UIElement relativeTo)
        {
        ...
        [Pure]
        private System.Windows.Point GetRelativePosition( VertexControl vertexControl)
        {
            Debug.Assert(vertexControl != null);
        ...
        [Pure]
        protected IDictionary<TVertex, Point> GetRelativePositions(bool continueLayout)
        {
            // If layout is continued it gets the relative position of every vertex
            // otherwise it's gets it only for the vertices with fixed parents
            ...
            else
            {
                foreach (KeyValuePair<TVertex, VertexControl> pair in VerticesControls.Where(pair => pair.Value is CompoundVertexControl))
                {
                    ...
                    foreach (var vertexControl in compoundVertexControl.Vertices)
                    {
        ...
        protected IDictionary<TVertex, Size> GetLatestVerticesSizes()
        {
            if (!IsMeasureValid)
            {
        ...
        private void ApplyVerticesPositions( IDictionary<TVertex, Point> positions)
        {
            // Animate the vertices
            foreach (var vertex in Graph.Vertices)
            {
                if (!VerticesControls.TryGetValue(vertex, out var control))
                    continue;

                if (positions.TryGetValue(vertex, out var pos))
                {
        ...
        private void ApplyEdgesRoutes( IDictionary<TEdge, Point[]> routeInfos)
        {
            // Change the edge routes
            foreach (var edge in Graph.Edges)
            {
                if (!EdgesControls.TryGetValue(edge, out var control))
                    continue;

                control.RoutePoints = routeInfos.TryGetValue(edge, out var routePoints)
